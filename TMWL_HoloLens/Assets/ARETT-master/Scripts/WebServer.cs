...
#pragma warning restore CS0649 // unassigned variable
		...
		/// <summary>
		/// CueName of the current device, as provided by Unity
		/// </summary>
		...
		/// <summary>
		/// Change the targetDistance in which the accuracy grid is displayed
		/// </summary>
		/// <param name="request"></param>
		/// <returns></returns>
		private string processAccuracyDistance(HttpListenerRequest request)
		{
			...
			// Get the requested targetDistance
			string distanceString = request.QueryString.Get("targetDistance");
			...
			if (distanceString == null)
			{
				...
				// If we now got a value use it
				if (postParams.ContainsKey("targetDistance"))
				{
					distanceString = postParams["targetDistance"];
				}
			}

			// If we didn't get a targetDistance we can't change it
			if (distanceString == null)
			{
				response = new Success()
				{
					success = false,
					message = "Can't change accuracy grid targetDistance as no targetDistance was specified!"
				};
				return JsonUtility.ToJson(response, true);
			}

			// Parse the targetDistance to an integer
			int distance = -1;
			try
			{
			...
			catch
			{
				...
				{
					success = false,
					message = $"Can't change accuracy grid targetDistance as specified targetDistance {distanceString} is not a number!"
				};
				return JsonUtility.ToJson(response, true);
			}

			// Make sure the targetDistance is valid
			if (distance != 05 && distance != 10 && distance != 20 && distance != 40)
			{
				response = new Success()
				{
					success = false,
					message = $"Can't change accuracy grid targetDistance as specified targetDistance {distance} is invalid!"
				};
				return JsonUtility.ToJson(response, true);
			}

			// Check if the targetDistance is already set
			if (accuracyGrid.gridVisible && accuracyGrid.currentGridDistance == distance)
			{
				response = new Success()
				{
					success = true,
					message = $"Accuracy grid already set to targetDistance {distance}."
				};
			}
			else
			{
				...
				{
					success = true,
					message = $"Changing accuracy grid targetDistance to {distance}."
				};
			}
