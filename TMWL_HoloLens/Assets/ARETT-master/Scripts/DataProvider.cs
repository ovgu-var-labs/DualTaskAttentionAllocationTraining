...
namespace ARETT
{
	...
	public class DataProvider : MonoBehaviour
	{
		...
		public Transform GazPointDebugger;
		public GameObject GazeTrailIndicator;
			...
			if (!dataQueue.IsEmpty)
			{
				while (dataQueue.TryDequeue(out GazeAPIData gazeAPIData))
				{
					...
					if (gazeAPIData.GazeHasValue)
					{
                        ...
                        GazPointDebugger.position =	Camera.main.transform.position + 2 * gazeAPIData.GazeDirection;
...
//							Instantiate(GazeTrailIndicator, hitInfo.point, Quaternion.identity);
							...
							GazPointDebugger.position = hitInfo.point;
						...
						if (gazeData.GazePointAOIHit)
						{
							...
							{
								manager.LookedAtTarget();
							}
							else if (hitInfo.collider.name.Contains("Body"))
							{
								manager.HitAOI(StudyManager.AOIType.Body);
							}
...
//							Instantiate(GazeTrailIndicator, hitInfo.point, Quaternion.identity);
						...
						{
                            manager.HitAOI(StudyManager.AOIType.None);
                        }
					}
